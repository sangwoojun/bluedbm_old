 ...
 *     #include "vpi_wrapper_can_put_tlp.h"
 *     void (*vlog_startup_routines[])() = { can_put_tlp_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_can_put_tlp call=can_put_tlp_calltf size=1 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_can_put_tlp vpiSysFuncSized 1 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
/* sft: $imported_can_put_tlp vpiSysFuncSized 1 unsigned */

/* tab: $imported_can_put_tlp call=can_put_tlp_calltf size=1 acc=rw:%TASK */

PLI_INT32 can_put_tlp_sizetf(PLI_BYTE8 *user_data)
{
  return 1u;
}
...
void can_put_tlp_vpi_register()
{
  ...
  tf_data.tfname = "$imported_can_put_tlp";
  tf_data.calltf = can_put_tlp_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = can_put_tlp_sizetf;
  tf_data.user_data = "$imported_can_put_tlp";
